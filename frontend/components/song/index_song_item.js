...
const SongIndexItem = props => {
    ...
    const button = (player === "playing" && song.id === currentSong.id) ?
        (<button onClick={handlePlayPause} className="pause-button-show">  <img src={window.pausebuttonurl} className="play-button-show-image" /> </button>)
        : (<button onClick={handlePlayPause} className="play-button-show">  <img src={window.playbuttonurl} className="play-button-show-image" /> </button>
    )
